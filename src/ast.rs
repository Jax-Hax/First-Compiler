...
impl AST {
  ...
  fn parse_identifier(&mut self, identifier: String) -> Node {
    ...
    match self.cur_token{
      Token::Other(char) => {if char != '(' {return Node::Variable(identifier)}},
    _ => {}
    }
    self.next_tok(); //eat (
    let args = vec![];
    match self.cur_token{
      Token::Other(char) => {if char != ')' {
        loop {
          ...
          match self.cur_token{
            Token::Other(char) => {if char == ')' {break;} else if char != ',' {return Node::Error("Expected ')' or ',' in an argument list".to_string())}},
          _ => {}
          }
          self.next_tok();
        }
    ...
    self.next_tok(); // eat )
    return Node::FunctionCall{name: identifier, args}
  }
  fn parse_parenthesis(&mut self) -> Node {
    ...
    match self.cur_token{
      Token::Other(char) => {if char != ')' {return Node::Error("Expected ')' but got {char}".to_string())}},
    _ => {}
    }
    self.next_tok(); //consume ')'
    return node.unwrap();
  ...
  fn parse_primary(&mut self) -> Node {
    match &self.cur_token{
      ...
      _ => {Node::Error(format!("Unknown token found when parsing expression: {:#?}", self.cur_token))}
    }
  }
}
pub fn construct_tree(tokens: IntoIter<Token>) -> Option<Vec<Node>> {
  ...
  loop {
    ...
    match node{
      ...
      Node::Error(err) => {eprintln!("Error: {}", err); return None;},
      _ => {}
    }
